...
// ログインは任意機能として始まったが、2026-09-26（㉘）以降、チェッカーはすべてログイン必須。
// /api/me が失敗・エラーになってもゲスト状態として扱う方針は変わらない（チェッカー本体の処理には影響させない）。
(function () {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // ⑮ ただし<body data-auth-stay>を指定したページ（ログイン必須のeternal-road.html。ログイン案内からログインした人を同じページに戻す）は、
      // トップへ遷移させず、その場でリロードしてログイン状態で再描画する
