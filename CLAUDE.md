...
Cowork側の設計資料`docs/㉔エタロ画像出力_自己紹介カード表記への統一_設計.md`に基づき実装した。付属の`docs/㉔eternal-road_改修参考実装.html`（現行`eternal-road.html`をベースにした反映済み全文）との差分が`generateShareImage()`周辺とシェア文1行のみであることを確認した上で取り込んだ。変更は`eternal-road.html`のみ（`worker.js`・DB・画面上の表記は変更なし）。
...
- データ登録状況バッジ（`.regBadge`）はエタロのカードには付けていない（依頼範囲外。必要なら`/api/my-eternal-road`の`registered`等から同様に実装可能）
...
- **ミッションの短い表示名`missionLabel()`は`eternal-road.html`（㉔）と自己紹介カード（㉒）で別々に持っている。ミッション種別を追加・変更するときは両方を同時に直すこと**
