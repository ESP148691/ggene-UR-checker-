...
### スクショ読み取りの本番化（`unit.html`へ統合・2026-09-26・ユーザーからの直接依頼）
「運営者専用のスクショ読み取りを本番化し、ユニット所持率チェッカーに反映。運営者ページからは削除」との指示。

- **`unit.html`**：`unit-scan.html`から読み取り機能を**スクリプトで移植**（手での書き写しなし）。移植したのは、CSS（一括選択ボタン含む）・導入ブロック`.scanIntro`（データ登録状況の下、所持率ゲージの上）・読み取り画面`#scanModal`／`#scanBar`（プレビューモーダルの後ろ）・`<script src="unit-scan.js">`と結果の確認・反映の即時関数（本体スクリプトの後ろ）。移植しなかったのは、運営者ガード（`adminGate`・`scanLocked`）・`noindex`・タイトルの「試験版」。「試験版」の表記（バッジ・見出し）も外した。**誰でも（ゲストも）使える**
- **入手記録との整合（追加）**：「写っていないユニットは『未所持』にする」で反映するとき、入手記録のあるユニットが外れる場合は他の操作と同じ`confirmAcqRemoval()`の確認を出す（キャンセルなら反映しない）。それ以外（凸の変更・新規）は入手記録に触れない
- **`top.html`**：運営者欄から「スクショ読み取り（試験版）」を削除（運営者欄は「X投稿用レポート」「自己紹介カード（試用）」の2つ）
...
- `auth.js`：冒頭コメントを更新。ログインモーダルのヒント文は当初「依頼書の範囲外」として未変更だったが、同日ユーザー指示（「ログインモーダルの文言はログイン必須に合わせて修正して」）で「各チェッカーは新規登録・ログインしてご利用いただけます（無料・メールアドレス不要。ユーザー名とパスワードだけで登録できます）。登録したデータはサーバーに保存され、機種変更や別の端末でも引き継げます。」に変更（旧：「登録すると自己紹介カードの保存・端末をまたいだデータ引き継ぎが利用できます（任意機能。未登録でもチェッカーは今まで通り使えます）。」）。文言はチェッカーのガードの案内文に合わせた
...
**段階B：自己紹介カード「標準」の改修・地球背景の月の削除（`profile-card.html`）**
- `CardRenderer`を`docs/04_自己紹介カード/試作HTML/㉗自己紹介カード_試作6_プロトタイプ.html`から**スクリプトで移植**（`BASE`だけ本番の`""`に戻した）。移植後、試作6との差分が`BASE`の1行だけであることを確認。変更は`layoutStandard()`（右カラム＝上・中段にURユニット＋推しユニット5機を約122px角で横一列、下段にサポート／エタロを左右半分。登録状況で全幅・上詰め・高さいっぱいに出し分け）と`bgEarth()`（遠くの月を削除）のみ。`layoutEternal()`・`layoutUnits()`・`missionChips()`は変更なし
- `buildCardData()`：`units.topOwned`（推しユニット未設定時に並べる所持ユニット。凸Lvの高い順・同じならID順に5機。形は`favoriteUnits`と同じ）を追加。`maxUnitIds`は未使用だが残置
...
**段階C：X投稿用レポートの6種類化（上位30位の2列ランキング）＋API追加**
- 経緯：最初は依頼書（2026-09-26当初版）どおり5種類・TOP10＋伸びTOP3で実装・push（`b047885`）。同日21:28に依頼書3章が更新され（ユーザー追加指示：恒常URユニットの追加、伸び率ランキングを廃止して上位30位の2列表示）、ユーザー確認のうえ段階Cの差分だけを追加実装した（段階D・A・Bは変更なし）
