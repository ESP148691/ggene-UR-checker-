...
- 仕様・DB設計・API・既知の制約は`../docs/WEBサイト仕様書.md`（Cowork側が新チャット開始時に読む現状スナップショット）にも重複して記載されている。ここに書いた変更でそちらの内容が古くなった場合（機能追加、制約の解消など）は、忘れずに`../docs/WEBサイト仕様書.md`側も合わせて更新すること
- Cowork側（claude.ai）で新しい仕様検討が行われた場合は`../docs/`に資料が追加される。作業開始時に`../docs/README.md`と、追加された資料がないか確認すること
