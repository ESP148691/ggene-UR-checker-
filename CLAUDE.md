...
- **既存不具合の指摘（未修正・ユーザー判断待ち）**：`restoreOwnershipFromServer()`は`data.ownership`が空オブジェクトでも真とみなすため、**ログイン中だが未登録のユーザーは、ページを開くたびにlocalStorageの所持状況が空で上書きされる**（⑪実装時からの挙動）。上書き条件に`data.registered`を加えれば解消するが、依頼範囲外のため変更していない
