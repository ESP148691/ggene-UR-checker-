...
### ⑧「トップに戻る」が効かない不具合（X経由）への対策（実装・検証・git commit・push完了・2026-09-20）
...
## 次にやること
- **⑧の実機確認（優先度高）**：本番反映済み。ユーザーにXアプリ内ブラウザで`docs/⑧トップに戻る不具合_原因調査と対策.md`記載の切り分け手順（`/top`直接アクセス・`/?a=1`アクセス）を試してもらい、症状が解消したか確認する
