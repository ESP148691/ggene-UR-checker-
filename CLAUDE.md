...
### ⑧「トップに戻る」が効かない不具合（X経由）への対策（コード実装・検証完了・2026-09-20。**git commit・push未実施**）
...
- **未対応（要判断）**：本対策はブラウザキャッシュ由来という仮説に基づく回避策であり、Xアプリ内ブラウザでの実機確認はできていない。`docs/⑧...md`記載の「ユーザー側での確認方法」（`/top`直接アクセス・`/?a=1`アクセスでの切り分け）を実機で試してもらい、症状が解消するか確認してもらう必要がある

## 次にやること
- **⑧のgit commit・push（優先度高）**：`auth.js`・`unit.html`・`supporter.html`・`analytics.html`・本ファイルの変更をコミット・プッシュし、本番環境（Cloudflare自動デプロイ）へ反映する。プッシュ後、ユーザーにXアプリ内ブラウザでの実機確認（`docs/⑧...md`記載の切り分け手順）を依頼すること
